...
import edu.stanford.isis.epad.common.pixelmed.SegmentationPropertyCategories;
import edu.stanford.isis.epad.common.pixelmed.SegmentationPropertyTypes;
...
public class SegmentationObjectsFileWriter
{
	...
	private final SequenceAttribute segment_sequence;
	private final SequenceAttribute shared_functional_groups_sequence;
	private final SequenceAttribute per_frame_functional_groups_sequence;
	private final Attribute pixel_data;
	private int current_segment;
	private int frame_counter;

	/**
	 * @param type is the segmentation type, either "BINARY" or "FRACTIONAL"; fractional_type is either "PROBABIITY" or
			...
			throws DicomException
	{
		...
		{
			Attribute a = new ShortStringAttribute(TagFromName.LossyImageCompression);
			a.addValue("00");
			list.put(a);
		}
		...
		if (seg_type.equalsIgnoreCase("BINARY")) { // 1-bit/pixel
			...
			if (fractional_type.equalsIgnoreCase("OCCUPANCY")) {
				...
				a.addValue("OCCUPANCY");
				list.put(a);
			} else {
				Attribute a = new CodeStringAttribute(TagFromName.SegmentationFractionalType);
				a.addValue("PROBABILITY");
				list.put(a);
			}
		}
		...
		{
			Attribute a = new LongStringAttribute(TagFromName.ContentCreatorName);
			a.addValue(ManufacturerModelName);
			list.put(a);
		}
		...
		SequenceAttribute pixel_measures_sequence = (SequenceAttribute)SequenceAttribute
				.getNamedAttributeFromWithinSequenceWithSingleItem(original_attrs, TagFromName.SharedFunctionalGroupsSequence,
						TagFromName.PixelMeasuresSequence);
		SequenceAttribute plane_orientation_sequence = (SequenceAttribute)SequenceAttribute
				.getNamedAttributeFromWithinSequenceWithSingleItem(original_attrs, TagFromName.SharedFunctionalGroupsSequence,
						TagFromName.PlaneOrientationSequence);
		...
		AttributeList reference = new AttributeList(); // Item of DerivationImageSequence.
		{
			...
			image.put(seq);
			{
				Attribute a = new LongStringAttribute(TagFromName.ReferencedSOPClassUID);
				a.addValue(orig_class_uid);
				image.put(a);
			}
			{
				Attribute a = new LongStringAttribute(TagFromName.ReferencedInstanceSequence);
		...
		}
		{
			...
			{
				Attribute a = new ShortStringAttribute(TagFromName.CodeMeaning);
		...
		if (pixel_measures_sequence != null) {
			Attribute spacing = SequenceAttribute.getNamedAttributeFromWithinSequenceWithSingleItem(pixel_measures_sequence,
					TagFromName.PixelSpacing);
			list.put(spacing);
			Attribute thickness = SequenceAttribute.getNamedAttributeFromWithinSequenceWithSingleItem(
					pixel_measures_sequence, TagFromName.SliceThickness);
			list.put(thickness);
		...
		if (plane_orientation_sequence != null) {
			Attribute a = SequenceAttribute.getNamedAttributeFromWithinSequenceWithSingleItem(plane_orientation_sequence,
	...
	public void AddOneSegment(int number, String description, String category, String type) throws DicomException
	{
		...
		{
			Attribute a = new LongStringAttribute(TagFromName.SegmentDescription);
		...
		{
			Attribute a = new LongStringAttribute(TagFromName.SegmentAlgorithmType);
		...
		}
		{
			...
			AttributeList item = new AttributeList();
			{
				Attribute a = new ShortStringAttribute(TagFromName.CodingSchemeName);
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			// Segment 1
			obj.AddOneSegment(1, "Segment No.1 is for ...", SegmentationPropertyCategories.Tissue,
					SegmentationPropertyTypes.Lung);
			obj.AddAllFrames(pixels, image_frames);

			// Segment 2
			obj.AddOneSegment(2, "Segment No.2 is for ...", SegmentationPropertyCategories.AnatomicalStructure,
					SegmentationPropertyTypes.AnkleJoint);
			obj.AddAllFrames(pixels, image_frames);
